<!DOCTYPE html>
<html>
<head>
<style>
	label{
		display: inline-block;
		margin: 5px;
		width: 100px;
	}
</style>
<link href="style.css" rel="stylesheet" type="text/css"></link>
</head>	
<body>
<?php
$Address_ID = $_GET ['Address_ID'];
require_once('dbconnection.php');

$sql = "SELECT * FROM address WHERE address_ID = '".$Address_ID."'";
$result = mysqli_query($con,$sql);
$row = mysqli_fetch_array($result);
$Address_ID = $row['address_id'];
$Address = htmlspecialchars($row['address']);
$District = htmlspecialchars($row['district']);
$City_ID =$row['city_id'];
$Postal_Code = $row['postal_code'];
$Phone = htmlspecialchars($row['phone']);

?>

<form id="insertform" action="Update Address Process.php" method="POST">
<label>Address ID : <b><?php print $Address_ID;?></b></label>
<input type = "hidden" name ="Address_ID" value ='<?php print $Address_ID;?>'><br>
<label>Address :</label>
<input name="Address" type = "text" value ='<?php print $Address;?>'><br>
<label>District :</label>
<input name="District" type = "text" value ='<?php print $District;?>'><br>
<label>City ID :</label>
<select name ="City_ID">
<?php
require_once('dbconnection.php');
$result = mysqli_query($con,"SELECT * FROM city");
while ($row = mysqli_fetch_array($result)){
    $city_id = $row['city_id'];
	$city = $row['city'];
	if ($city_id == $City_ID) {
    echo "<option value ='$city_id' selected>$city</option>";
	}
	else{
	echo "<option value ='$city_id'>$city</option>";
	}
}
?>
</select><br>
<label>Postal Code :</label>
<input name="Postal_Code" type = "text" value ='<?php print $Postal_Code;?>'><br>
<label>Phone Number :</label>
<input name="Phone" type = "text" value ='<?php print $Phone;?>'><br>
<label> <u style="text-decoration-thickness: 3px;">Current Time</u></label><br>
<div id="date"></div>
<div id="clock"></div>
	<script>
				function Time() {
					//Code section for clock
					var date = new Date();
					var hour = date.getHours();
					var min = date.getMinutes();
					var sec = date.getSeconds();
					hour = updateTime( hour );
					min = updateTime( min );
					sec = updateTime( sec );
					var timezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
					document.getElementById( "clock" ).innerText = hour + " : " + min + " : " + sec + " ( Timezone : " + timezone + " )";
					var update = setTimeout( function () {
						Time()
					}, 1000 );

					//Code section for date
					months = [ "January", "February", "March", "April", "May", "June",
						"July", "August", "September", "October", "November", "December"
					];

					days = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday" ];

					var year = date.getFullYear();
					var month = date.getMonth();
					var day = date.getDay();
					var day_date = date.getDate();
					document.getElementById( "date" ).innerText = days[ day ] + " " + months[ month ] + " " + day_date + ordinal_checker( day_date ) + " " + year;
				}

				function updateTime( k ) {
					if ( k < 10 ) {
						return "0" + k;
					} else {
						return k;
					}
				}

				function ordinal_checker( day_date ) {
					switch ( day_date ) {
						case 1:
							ordinal = "st"
							return ordinal
							break;

						case 2:
							ordinal = "nd"
							return ordinal
							break;

						case 3:
							ordinal = "rd"
							return ordinal
							break;

						default:
							ordinal = "th"
							return ordinal
					}
				}
				Time();
	</script>
<h5><input id="submit" type = "submit" name = "edit" value = "Update">
<button id="button" type="button" onclick="location.href = 'Index.html'">Back</button></h5>
</form>

</body>
</html>