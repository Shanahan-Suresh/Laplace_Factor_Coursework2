...
$sql = "SELECT * FROM film_actor WHERE actor_id = '".$Actor_ID."'";
$result = mysqli_query($con,$sql);
$row = mysqli_fetch_array($result);
$Film_ID = htmlspecialchars($row['film_id']);
$Actor_ID = $row['actor_id'];
?>

<form id="insertform" action="Update Film_Actor Process.php" method="POST">
<label>ID : <b><?php print $Actor_ID;?></b></label>
<input type = "hidden" name ="Actor_ID" value ='<?php print $Actor_ID;?>'><br>
<label>Film_ID :</label>
<select name="Film_ID">
<?php
require_once('dbconnection.php');
...
while ($row = mysqli_fetch_array($result)){
	...
	$title = $row['title'];
	if ($film_id == $Film_ID) {
    echo "<option value ='$film_id' selected>$title</option>";
	}
	else{
